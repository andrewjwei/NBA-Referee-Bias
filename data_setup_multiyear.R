...
for(index in 1:8)
{
  ...
  L2M_Dataset <- data.frame(SCOREMARGIN = pbp$SCOREMARGIN,
                         MINS = pbp$MINS,
                         PERIOD = pbp$PERIOD,
                         GAME_ID = pbp$GAME_ID,
                         SECS = pbp$SECS,
  ...
  L2M_Dataset <- L2M_Dataset[(L2M_Dataset$MINS < 2) & (L2M_Dataset$PERIOD == 4),]
  
  # then, filter on score margin
  
  L2M_Dataset$KEEP <- FALSE
  L2M_Dataset$SCOREMARGIN <-L2M_Dataset$SCOREMARGIN %>% as.character()
  L2M_Dataset$SCOREMARGIN[L2M_Dataset$SCOREMARGIN == "TIE"] = "0"
  L2M_Dataset$SCOREMARGIN <-L2M_Dataset$SCOREMARGIN %>% as.numeric()
  
  ## check score margin
    
  Box_Data$l2m_secs <- 0
  
  for(i in 1:length(unique(L2M_Dataset$GAME_ID))){
    game = unique(L2M_Dataset$GAME_ID)[i]
    if(sum(L2M_Dataset$SCOREMARGIN[L2M_Dataset$GAME_ID == game] %>% abs() < 5, na.rm = T) > 0)
    {
      L2M_Dataset$KEEP[L2M_Dataset$GAME_ID == game] <- TRUE
      pbp_interval = (pbp$GAME_ID == game) & (pbp$MINS < 2) & (pbp$PERIOD == 4)
      prior_time <- 120
      for(j in 1:sum(pbp_interval))
      {
        if(!(pbp$SUB_ENTERED_PLAYER_ID[pbp_interval][j] %>% is.na()))
        {
          Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_1[pbp_interval][j]] = 
            Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_1[pbp_interval][j]] + prior_time - pbp$curr_time[pbp_interval][j]
          Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_2[pbp_interval][j]] = 
            Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_2[pbp_interval][j]] + prior_time - pbp$curr_time[pbp_interval][j]
          Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_3[pbp_interval][j]] = 
            Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_3[pbp_interval][j]] + prior_time - pbp$curr_time[pbp_interval][j]
          Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_4[pbp_interval][j]] = 
            Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_4[pbp_interval][j]] + prior_time - pbp$curr_time[pbp_interval][j]
          Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_5[pbp_interval][j]] = 
            Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_5[pbp_interval][j]] + prior_time - pbp$curr_time[pbp_interval][j]
          Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_1[pbp_interval][j]] = 
            Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_1[pbp_interval][j]] + prior_time - pbp$curr_time[pbp_interval][j]
          Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_2[pbp_interval][j]] = 
            Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_2[pbp_interval][j]] + prior_time - pbp$curr_time[pbp_interval][j]
          Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_3[pbp_interval][j]] = 
            Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_3[pbp_interval][j]] + prior_time - pbp$curr_time[pbp_interval][j]
          Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_4[pbp_interval][j]] = 
            Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_4[pbp_interval][j]] + prior_time - pbp$curr_time[pbp_interval][j]
          Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_5[pbp_interval][j]] = 
            Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_5[pbp_interval][j]] + prior_time - pbp$curr_time[pbp_interval][j]
      ...
      Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_1[pbp_interval][j]] = 
        Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_1[pbp_interval][j]] + prior_time
      Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_2[pbp_interval][j]] = 
        Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_2[pbp_interval][j]] + prior_time
      Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_3[pbp_interval][j]] = 
        Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_3[pbp_interval][j]] + prior_time
      Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_4[pbp_interval][j]] = 
        Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_4[pbp_interval][j]] + prior_time
      Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_5[pbp_interval][j]] = 
        Box_Data$l2m_secs[Box_Data$player_id == pbp$HOME_PLAYER_ID_5[pbp_interval][j]] + prior_time
      Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_1[pbp_interval][j]] = 
        Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_1[pbp_interval][j]] + prior_time
      Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_2[pbp_interval][j]] = 
        Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_2[pbp_interval][j]] + prior_time
      Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_3[pbp_interval][j]] = 
        Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_3[pbp_interval][j]] + prior_time
      Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_4[pbp_interval][j]] = 
        Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_4[pbp_interval][j]] + prior_time
      Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_5[pbp_interval][j]] = 
        Box_Data$l2m_secs[Box_Data$player_id == pbp$AWAY_PLAYER_ID_5[pbp_interval][j]] + prior_time
      
    }
    else
    {
      #do not keep
      L2M_Dataset$KEEP[L2M_Dataset$GAME_ID == game] <- FALSE
    }
  }
  

  L2M_Dataset$SCOREMARGIN <- NULL
  L2M_Dataset <- L2M_Dataset %>% filter(KEEP == TRUE)
  L2M_Dataset$KEEP <- NULL
  ...
  LQ_Dataset <- LQ_Dataset[(LQ_Dataset$PERIOD == 4),]
  ...
  for(i in 1:length(unique(LQ_Dataset$GAME_ID))){
    ...
    if(sum(LQ_Dataset$SCOREMARGIN[LQ_Dataset$GAME_ID == game] %>% abs() < 10, na.rm = T) > 0)
    {
      LQ_Dataset$KEEP[LQ_Dataset$GAME_ID == game] <- TRUE
      pbp_interval = (pbp$GAME_ID == game) & (pbp$PERIOD == 4)
      prior_time <- 720
      if(sum(pbp_interval, na.rm = T) == 0){ next }
      for(j in 1:sum(pbp_interval))
      {
        if(!(pbp$SUB_ENTERED_PLAYER_ID[pbp_interval][j] %>% is.na()))
        {
  ...
  Box_Data <- L2M_Dataset %>% count(DRAWN_FOUL_PLAYER_ID) %>% rename(c("player_id" = "DRAWN_FOUL_PLAYER_ID", "fouls_drawn_l2m" = "n")) %>% full_join(Box_Data, by = "player_id")
  Box_Data <- L2M_Dataset %>% count(FOULED_BY_PLAYER_ID) %>% rename(c("player_id" = "FOULED_BY_PLAYER_ID", "fouls_committed_l2m" = "n")) %>% full_join(Box_Data, by = "player_id")
  Box_Data <- L2M_Dataset %>% count(REBOUND_PLAYER_ID) %>% rename(c("player_id" = "REBOUND_PLAYER_ID", "reb_l2m" = "n")) %>% full_join(Box_Data, by = "player_id")
  Box_Data <- L2M_Dataset %>% count(STEAL_PLAYER_ID) %>% rename(c("player_id" = "STEAL_PLAYER_ID", "stl_l2m" = "n")) %>% full_join(Box_Data, by = "player_id")
  Box_Data <- L2M_Dataset %>% count(BLOCK_PLAYER_ID) %>% rename(c("player_id" = "BLOCK_PLAYER_ID", "blk_l2m" = "n")) %>% full_join(Box_Data, by = "player_id")
  
  Box_Data <- LQ_Dataset %>% count(DRAWN_FOUL_PLAYER_ID) %>% rename(c("player_id" = "DRAWN_FOUL_PLAYER_ID", "fouls_drawn_lq" = "n")) %>% full_join(Box_Data, by = "player_id")
  Box_Data <- LQ_Dataset %>% count(FOULED_BY_PLAYER_ID) %>% rename(c("player_id" = "FOULED_BY_PLAYER_ID", "fouls_committed_lq" = "n")) %>% full_join(Box_Data, by = "player_id")
  Box_Data <- LQ_Dataset %>% count(REBOUND_PLAYER_ID) %>% rename(c("player_id" = "REBOUND_PLAYER_ID", "reb_lq" = "n")) %>% full_join(Box_Data, by = "player_id")
  Box_Data <- LQ_Dataset %>% count(STEAL_PLAYER_ID) %>% rename(c("player_id" = "STEAL_PLAYER_ID", "stl_lq" = "n")) %>% full_join(Box_Data, by = "player_id")
  Box_Data <- LQ_Dataset %>% count(BLOCK_PLAYER_ID) %>% rename(c("player_id" = "BLOCK_PLAYER_ID", "blk_lq" = "n")) %>% full_join(Box_Data, by = "player_id")
...
saveRDS(Box_Data, file = "boxdata.rds")
